...
impl ItemRepository for SqliteItemRepo {
    ...
    async fn update_item_status(&self, user_id: Uuid, id: Uuid, status: TaskStatus) -> AppResult<()> {
        let result = sqlx::query(
            ...
            .bind(status.as_str()).bind(id.to_string()).bind(user_id.to_string()).execute(&self.pool).await?;
        if result.rows_affected() != 1 {
            return Err(crate::error::AppError::InvalidInput(format!(
    ...
    async fn update_sync_status(&self, user_id: Uuid, id: uuid::Uuid, sync_status: &str) -> AppResult<()> {
        let result = sqlx::query(
            "UPDATE items SET sync_status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?"
        )
        .bind(sync_status)
        .bind(id.to_string())
        .bind(user_id.to_string())
    ...
    async fn update_item_details(&self, user_id: Uuid, id: Uuid, title: String, description: Option<String>, due: Option<DateTime<Utc>>, duration_minutes: Option<i32>, motivation: i8) -> AppResult<()> {
        ...
        .bind(title).bind(description).bind(due).bind(duration_minutes).bind(motivation).bind(id.to_string()).bind(user_id.to_string())
        .execute(&self.pool).await?;
        Ok(())
    }
    ...
    async fn archive_item(&self, user_id: Uuid, id: Uuid) -> AppResult<()> {
        ...
        .bind(id.to_string()).bind(user_id.to_string()).execute(&self.pool).await?;
        Ok(())
    }
