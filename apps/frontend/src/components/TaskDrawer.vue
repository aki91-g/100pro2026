...
function handleListSelectItem(item: Item): void {
  emit('select-item', item);
}

function handleListEditItem(item: Item): void {
  emit('select-item', item);
  setMode('edit');
}
...
<template>
  ...
  >
    <div v-if="open" class="fixed inset-0 z-40">
      <div class="absolute inset-0 bg-slate-950/25 backdrop-blur-sm" @click="closeDrawer" />
      ...
      <aside role="dialog" aria-modal="true" class="absolute right-0 top-0 flex h-full w-full max-w-xl flex-col border-l border-slate-200 bg-white shadow-2xl">
        ...
        <div class="flex-1 overflow-y-auto p-5">
          ...
          >
            <form v-if="mode === 'create'" key="create" @submit.prevent="submitCreate" class="space-y-5">
              <div>
                <label for="create-title" class="mb-1.5 block text-[10px] font-black tracking-widest text-slate-400">TITLE *</label>
                <input
                  id="create-title"
                  v-model="createTitle"
                  type="text"
                  ...
                  :disabled="isCreating"
                  class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 focus:border-slate-900 focus:outline-none"
                />
              </div>

              <div>
                <label for="create-description" class="mb-1.5 block text-[10px] font-black tracking-widest text-slate-400">DESCRIPTION</label>
                <textarea
              ...
              <div>
                <label for="create-due" class="mb-1.5 block text-[10px] font-black tracking-widest text-slate-400">DUE *</label>
                <input
                  id="create-due"
                  v-model="createDue"
                  type="datetime-local"
              ...
              <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <label for="create-duration" class="mb-1.5 block text-[10px] font-black tracking-widest text-slate-400">DURATION (MIN)</label>
                  <input
                    id="create-duration"
                ...
                <div>
                  <label for="create-motivation" class="mb-1.5 block text-[10px] font-black tracking-widest text-slate-400">MOTIVATION</label>
                  <select
            ...
            <div v-else-if="mode === 'view'" key="view" class="space-y-4">
              <div v-if="selectedItem" class="rounded-xl border border-slate-200 bg-white p-4">
                <div class="flex items-start justify-between gap-3">
                  ...
                  <button
                    type="button"
                    class="rounded-md bg-slate-900 px-2.5 py-1.5 text-xs font-semibold text-white hover:bg-slate-800"
                    @click="setMode('edit')"
                  >
                    Edit
                  </button>
                ...
                <div class="mt-3 grid grid-cols-2 gap-3 text-xs text-slate-600">
                  <div>
                    <span class="text-[10px] font-black tracking-widest text-slate-400">STATUS</span>
                    <p class="mt-1">{{ selectedItem.status }}</p>
                  </div>
                  <div>
                    <span class="text-[10px] font-black tracking-widest text-slate-400">DUE</span>
                    <p class="mt-1">{{ new Date(selectedItem.due).toLocaleString() }}</p>
                  </div>
                  <div>
                    <span class="text-[10px] font-black tracking-widest text-slate-400">MOTIVATION</span>
                    <p class="mt-1">{{ selectedItem.motivation ?? 'None' }}</p>
                  </div>
                  <div>
                    <span class="text-[10px] font-black tracking-widest text-slate-400">DURATION</span>
              ...
              <div v-else class="rounded-xl border border-dashed border-slate-300 bg-white p-4 text-sm text-slate-500">
                Select a task from the list below to view details.
              </div>
              ...
              <TaskList
                ...
                @select-item="handleListSelectItem"
                @edit-item="handleListEditItem"
              />
            </div>

            <form v-else key="edit" class="flex h-full flex-col gap-5" @submit.prevent="handleEditSubmit">
              <div>
                <label for="edit-title" class="mb-1.5 block text-[10px] font-black tracking-widest text-slate-400">TITLE</label>
                <input
                  id="edit-title"
                  v-model="editTitle"
                  type="text"
              ...
              <div>
                <label for="edit-description" class="mb-1.5 block text-[10px] font-black tracking-widest text-slate-400">DESCRIPTION</label>
                <textarea
                  id="edit-description"
              ...
              <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  <label for="edit-due" class="mb-1.5 block text-[10px] font-black tracking-widest text-slate-400">DUE</label>
                  <input
                    id="edit-due"
                    v-model="editDue"
                    type="datetime-local"
                ...
                <div>
                  <label for="edit-duration" class="mb-1.5 block text-[10px] font-black tracking-widest text-slate-400">DURATION</label>
                  <input
                    id="edit-duration"
              ...
              <div>
                <label for="edit-motivation" class="mb-1.5 block text-[10px] font-black tracking-widest text-slate-400">MOTIVATION</label>
                <select
                  id="edit-motivation"
